...
if st.session_state.stellar_data:
    ...
    filter_mode = st.radio("Selection Mode", ["By Month/Week", "Custom Date Range"], horizontal=True)
    
    t1, t2, t3 = st.columns(3)
    start_date, end_date = None, None

    if filter_mode == "By Month/Week":
        with t1:
        ...
        with t2:
            if sel_month == "All Months":
                ...
                month_idx = list(calendar.month_name).index(month_parts[0])
                year_val = int(month_parts[1])
                _, last_day = calendar.monthrange(year_val, month_idx)
        ...
        with t1:
            date_range = st.date_input("Select Date Range", value=(df['timestamp'].min().date(), df['timestamp'].max().date()))
    ...
    if not selected_assets:
        ...
        # Fixed Sorting Logic: Ascending=True for "Ascending", False for "Descending"
