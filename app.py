...
# Initialize filter states to maintain them across navigation
...
def load_account_data(identifier, months, save_to_history=True):
    # If we are navigating to a NEW account, save the OLD one to the stack first
    ...
    with st.spinner(f"Loading data for {identifier}..."):
        target_id = None
...
# Trigger logic if URL contains a different account than current state
...
if st.session_state.history_stack:
    ...
    if st.button(f"← Back to {prev['name']}", key="back_button"):
        # Pop the last item and load it
...
if st.session_state.stellar_data:
    ...
    if filter_mode == "Standard (Month/Week)":
        with t1:
            ...
            sel_month = st.selectbox("Filter by Month", ["All Months"] + available_months, key="sel_month")
        with t2:
            if sel_month == "All Months":
                ...
                sel_week = st.selectbox("Filter by Week", ["All Weeks"] + dynamic_weeks, key="sel_week")
    else:
        with t1:
    ...
    if not selected_assets:
        ...
        display_df['Linked Account'] = display_df.apply(create_link, axis=1)
        display_df['Amount_Disp'] = display_df.apply(lambda r: f"{r['amount']:,.2f}" if r['asset'] == "DMMK" else f"{r['amount']:,.7f}", axis=1)
        
        st.write("**Transaction History**")
        st.markdown(display_df[['Date/Time', 'direction', 'Linked Account', 'Amount_Disp', 'asset']].rename(columns={'direction':'Direction','Linked Account':'Other Account','Amount_Disp':'Amount','asset':'Asset'}).to_html(escape=False, index=False, classes="dataframe"), unsafe_allow_html=True)
        
        # --- EXPORT TRANSACTION HISTORY ---
        csv_history = filtered_df[['timestamp', 'direction', 'other_account', 'other_account_id', 'amount', 'asset']].to_csv(index=False).encode('utf-8')
        st.download_button("⬇️ Export Transaction History (CSV)", csv_history, f"{st.session_state.display_name}_transactions.csv", "text/csv")
        ...
        disp_sum['Linked Account'] = disp_sum.apply(create_link, axis=1)
        for c in ['Total_Volume', 'Incoming', 'Outgoing', 'Net_Difference']: disp_sum[c] = disp_sum[c].apply(lambda x: f"{x:,.2f}")
        
        st.markdown(disp_sum[['Linked Account', 'asset', 'Total_Volume', 'Incoming', 'Outgoing', 'Net_Difference', 'Tx_Count']].rename(columns={'Linked Account':'Other Account','asset':'Asset','Total_Volume':'Total Volume','Net_Difference':'Net Balance','Tx_Count':'Tx Count'}).to_html(escape=False, index=False, classes="dataframe"), unsafe_allow_html=True)
